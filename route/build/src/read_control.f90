...
CONTAINS
 ...
 do iLine=1,size(cLines)
   ...
   case('<doesSubSurfRoute>');     read(cData,*,iostat=io_error) doesSubSurfRoute  ! option to route subsurface runoff 0-> no, 1-> yes.
   ...
   case('<vname_subqsim>');        vname_subqsim= trim(cData)                      ! name of subsurface variable
  ...
  !  case('<varname_minFlow>'        ); meta_SEG    (ixSEG%minFlow         )%varName =trim(cData)   ! minimum environmental flow
   ...
   case('<varname_D03_MinStorage>' ); meta_SEG    (ixSEG%D03_MinStorage  )%varName =trim(cData)   ! Doll 2006; minimum active storage for Doll 2003 formulation
