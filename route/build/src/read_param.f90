 ...
 CONTAINS
  ...
  SUBROUTINE read_param(fname, ierr, message)
   ...
   USE globalData,        ONLY: fshape, tscale, &  ! basin IRF routing parameters
                                subfshape, subtscale,& ! subbasin routing parameters
   ...
   namelist /SUBSURFACE/subfshape,subtscale  ! route simulated runoff through the local basin
   ...
   if (doesSubSurfRoute==1) then
    read(iunit, nml=SUBSURFACE)          ! basin subsurface routing parameters
   endif
