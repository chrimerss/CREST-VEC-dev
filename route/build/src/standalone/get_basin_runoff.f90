...
CONTAINS
 ...
 ! populate runoff_data with runoff values at LSM domain and at iTime step
  ...
  USE public_var,  ONLY:vname_subqsim           ! varibale subsurface runoff in netCDF file
  ...
  ! get the simulated subsurface runoff for the current time step - runoff_data%simSub(:) or %sim2DSub(:,:)
  call read_runoff_data(trim(input_dir)//trim(fname_qsim), & ! input: filename
                        trim(vname_subqsim),                  & ! input: varname
                        iTime_local,                       & ! input: time index
                        runoff_data%nSpace,                & ! inout: runoff data structure
                        runoff_data%simSub,                   & ! inout: runoff data structure
                        runoff_data%sim2DSub,                 & ! inout: runoff data structure
                        ierr, cmessage)                      ! output: error control                        
  if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
  if ( allocated(runoff_data%basinSubRunoff)  ) then
    deallocate(runoff_data%basinSubRunoff, stat=ierr)
    if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
  endif
  allocate(runoff_data%basinSubRunoff(nHRU), stat=ierr)
  if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif 
  endif
  ...
  if (is_remap) then ! remap LSM simulated flux to the HRUs in the river network
    ...
    if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    if (doesSubSurfRoute==1)then
      call remap_runoff(runoff_data, remap_data, runoff_data%basinSubRunoff, ierr, cmessage)
      if (ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    endif
