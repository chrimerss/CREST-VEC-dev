...
CONTAINS
 ...
 SUBROUTINE init_time(ierr, message)
  ...
  if (nTime > 1) then
    ...
    if ( any(abs(roJulday_diff-roJulday_diff(1)) > 1.e-6_dp) ) then
      write(iulog,'(2a, F12.7)') new_line('a'),'ERROR: contacenated netCDF files have time overlaps or gap, difference=', maxval(abs(roJulday_diff-roJulday_diff(1)))
  ...
  if ((is_flux_wm).or.(is_vol_wm.and.is_lake_sim)) then
    ...
    if (nTime_wm > 1) then
      ...
      ! check if the difference are identical otherwise error and terminate
      if ( any(abs(roJulday_diff_wm-roJulday_diff_wm(1)) > verySmall) ) then
 ...
 USE public_var,  ONLY: vname_subqsim          ! name of simulated runoff varibale 
 ...
 USE public_var,  ONLY: doesSubSurfRoute       ! logical whether or not target volume should be read
 ...
 call read_runoff_metadata(trim(input_dir)//trim(fname_qsim),  & ! input: filename
                           vname_subqsim,                      & ! input: varibale name for simulated runoff
                           vname_time,                         & ! input: varibale name for time
                           dname_time,                         & ! input: dimension of variable time
                           vname_hruid,                        & ! input: varibale hruid
                           dname_hruid,                        & ! input: dimension of varibale hru
                           ...
                           runoff_data_in%nSpace,              & ! nSpace of the input in runoff or wm strcuture
                           runoff_data_in%nTime,               & ! nTime of the input in runoff or wm strcuture
                           runoff_data_in%simSub,              & ! 1D simulation
                           runoff_data_in%sim2DSub,            & ! 2D simulation
                           runoff_data_in%hru_id,              & ! ID of seg or hru in data
                           time_units, calendar,               & ! output: number of time steps, time units, calendar
                           ierr, cmessage)                       ! output: error control
 if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
