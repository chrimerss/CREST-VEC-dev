module lake_route_module
 ...
 private
  ...
  ! print*, 'inside lake, time at the model simulation',simDatetime(1)%year(),simDatetime(1)%month(),simDatetime(1)%day(),simDatetime(1)%hour(),simDatetime(1)%minute(),simDatetime(1)%sec()
    ...
    ! print*, 'lake param D03_MinStorage ..= ', RPARAM_in(segIndex)%D03_MinStorage
    ...
    if (NETOPO_in(segIndex)%LakeTargVol) then ! The lake should follow the given target volume
      ...
      select case(NETOPO_in(segIndex)%LakeModelType)
        ...
        case (1)
          if (RPARAM_in(segIndex)%D03_MinStorage>=RPARAM_in(segIndex)%D03_MaxStorage) then
            RPARAM_in(segIndex)%D03_MinStorage=0 !minimal storage cannot go beyond maximum storage
          endif
          ! the model is Doll03
          ! print*, "lake model is Doll 2003"
          ! print*, RPARAM_in(segIndex)%D03_Coefficient, RPARAM_in(segIndex)%D03_Power
          RCHFLX_out(iens,segIndex)%REACH_Q_IRF = RPARAM_in(segIndex)%D03_Coefficient * RCHFLX_out(iens,segIndex)%REACH_VOL(1) * &
                                                   ((RCHFLX_out(iens,segIndex)%REACH_VOL(1)-RPARAM_in(segIndex)%D03_MinStorage) /&
                                                  (RPARAM_in(segIndex)%D03_MaxStorage-RPARAM_in(segIndex)%D03_MinStorage) ) ** &
                                                   RPARAM_in(segIndex)%D03_Power! Q = AS((S-Smin)/(Smax-Smin))^B based on Eq. 1 Hanasaki et al., 2006 https://doi.org/10.1016/j.jhydrol.2005.11.011
          ! in case is the output volume is more than lake volume
          ! print*, RPARAM_in(segIndex)%D03_MinStorage, RPARAM_in(segIndex)%D03_MaxStorage, RPARAM_in(segIndex)%D03_Coefficient * RCHFLX_out(iens,segIndex)%REACH_VOL(1) * &
          !                                          ((RCHFLX_out(iens,segIndex)%REACH_VOL(1)-RPARAM_in(segIndex)%D03_MinStorage) /&
          !                                         (RPARAM_in(segIndex)%D03_MaxStorage-RPARAM_in(segIndex)%D03_MinStorage) ) ** &
          !                                          RPARAM_in(segIndex)%D03_Power, RPARAM_in(segIndex)%D03_Coefficient * RCHFLX_out(iens,segIndex)%REACH_VOL(1) * &
          !                                          ((RCHFLX_out(iens,segIndex)%REACH_VOL(1)) /&
          !                                         (RPARAM_in(segIndex)%D03_MaxStorage) ) ** &
          !                                          RPARAM_in(segIndex)%D03_Power
          ...
          ! cannot exceed maximum storage
          RCHFLX_out(iens,segIndex)%REACH_VOL(1) = RCHFLX_out(iens,segIndex)%REACH_VOL(1) - RCHFLX_out(iens,segIndex)%REACH_Q_IRF * dt
          ! RCHFLX_out(iens, segIndex)%REACH_VOL(1) = min(RCHFLX_out(iens, segIndex)%REACH_VOL(1),
          if (RCHFLX_out(iens,segIndex)%REACH_VOL(1) < 0) then; ! set the lake volume as 0 if it goes negative
            RCHFLX_out(iens,segIndex)%REACH_VOL(1)=0
          endif
          if (RCHFLX_out(iens,segIndex)%REACH_VOL(1) > RPARAM_in(segIndex)%D03_MaxStorage .and. RPARAM_in(segIndex)%D03_MaxStorage>1)then 
            ! print*, 'before correction: ', RCHFLX_out(iens,segIndex)%REACH_Q_IRF
            RCHFLX_out(iens, segIndex)%REACH_Q_IRF= RCHFLX_out(iens, segIndex)%REACH_Q_IRF+(RCHFLX_out(iens,segIndex)%REACH_VOL(1)-RPARAM_in(segIndex)%D03_MaxStorage)/dt
            ! print*, 'after correction: ', RCHFLX_out(iens,segIndex)%REACH_Q_IRF
            ! print*, 'increased by ', (RCHFLX_out(iens,segIndex)%REACH_VOL(1)-RPARAM_in(segIndex)%D03_MaxStorage)/dt
    ...
    !     if (segIndex==358)then 
    !  print*, 'volume before simulation m3.= ', RCHFLX_out(iens,segIndex)%REACH_VOL(1)
    !  endif 
