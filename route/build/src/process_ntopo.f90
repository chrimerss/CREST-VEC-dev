...
contains
 ...
 subroutine put_data_struct(nSeg, structSEG, structNTOPO, &
  ...
  USE globalData,    ONLY: fshape,subfshape,&
                           tscale,subtscale    ! basin IRF routing parameters (Transfer function parameters)
  USE public_var,    ONLY: min_slope          ! minimum slope
  USE public_var,    ONLY: dt                 ! simulation time step [sec]
  ...
  call basinUH(dt, fshape, subfshape, tscale, subtscale, ierr, cmessage)
  if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
  ...
  do iSeg=1,nSeg
   ...
   if (is_lake_sim) then
     ...
     RPARAM_in(iSeg)%D03_MinStorage  =     structSEG(iSeg)%var(ixSEG%D03_MinStorage)%dat(1)
