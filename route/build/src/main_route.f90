...
CONTAINS
 ...
 SUBROUTINE main_route(&
                       ...
                       basinSubRunoff_in, &  ! basin subsurface runoff (m/s)                       
   ...
   USE public_var, ONLY: doesBasinRoute, doesSubSurfRoute
   ...
   real(dp),           allocatable, intent(in)    :: basinSubRunoff_in(:) ! basin (i.e.,HRU) runoff (m/s)
   ...
   real(dp),           allocatable                :: reachSubRunoff_local(:) ! reach runoff (m/s)
  ...
  allocate(reachSubRunoff_local(nSeg), stat=ierr)
  ...
  if (doesSubSurfRoute==1) then 
    call basin2reach(basinSubRunoff_in,     & ! input: basin subsurface runoff (m/s)
                     NETOPO_in,             &
                     RPARAM_in,             &
                     reachSubRunoff_local,  &
                     ierr, cmessage,        &
                     ixRchProcessed)
  end if
  ...
  if (doesBasinRoute == 1) then
    ...
    if (doesSubSurfRoute==1) then
        do iSeg = 1,nSeg
          RCHFLX_SUB(iens,ixRchProcessed(iSeg))%BASIN_QI = reachSubRunoff_local(iSeg)
        enddo
        if (SUM(RCHFLX_sub(iens, 1:nSeg)%BASIN_QI)==0)then
          print*, 'Warning: zero subrunoff input to be routed...' 
        endif 
        call IRF_route_basin_subsurf(iens,                       & ! ensemble index
                                     NETOPO_in,                  & ! input: reach topology
                                     RCHFLX_SUB,                 & ! inout: reach flux data structure plus subsurface routing
                                     ierr,cmessage,              & ! output: error handles
                                     ixRchProcessed)               ! optional input: indices of reach to be routed
        if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
        if (SUM(RCHFLX_SUB(iens, 1:nSeg)%BASIN_QR(0))==0)then
          print*, 'Warning: zero subrunoff is routed...' 
        endif        
        do iSeg=1, nSeg
            RCHFLX_out(iens,ixRchProcessed(iSeg))%BASIN_QR(0)= RCHFLX_out(iens,ixRchProcessed(iSeg))%BASIN_QR(0)+RCHFLX_SUB(iens,ixRchProcessed(iSeg))%BASIN_QR(0)
            RCHFLX_out(iens,ixRchProcessed(iSeg))%BASIN_QR(1)= RCHFLX_out(iens,ixRchProcessed(iSeg))%BASIN_QR(1)+RCHFLX_SUB(iens,ixRchProcessed(iSeg))%BASIN_QR(1)
        enddo
    endif 
