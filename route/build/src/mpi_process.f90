...
contains
 ...
 subroutine comm_ntopo_data(pid,                & ! input: proc id
  ...
  real(dp),          allocatable              :: D03_MinStorage_local(:)   ! Doll 2003; maximum active storage for Doll 2003
  ...
  real(dp)                                    :: D03_MinStorage(nRch_in)   ! Doll 2003; maximum active storage for Doll 2003
  ...
  if (masterproc) then ! this is a root process
    ...
    do iSeg = 1,nRch_in
     ...
     if (is_lake_sim) then
       ...
       D03_MinStorage(iSeg)  = structSEG(  jSeg)%var(ixSEG%D03_MinStorage)%dat(1)
  ...
  if (nNodes>1) then
    ...
    if (is_lake_sim) then
      ...
      call shr_mpi_scatterV(D03_MinStorage        (nRch_mainstem+1:nRch_in), rch_per_proc(0:nNodes-1), D03_MinStorage_local, ierr, cmessage)
    ...
    reach: do ix = 1,rch_per_proc(pid)
     ...
     if (is_lake_sim) then
       ...
       structSEG_local  (ix)%var(ixSEG%D03_MinStorage)%dat(1)   = D03_MinStorage_local(ix)
...
!       endif
   ...
   if (masterproc) then
     ...
     main_rch: do ix = 1, nRch_mainstem
       ...
       if (is_lake_sim) then
         ...
         structSEG_main  (ix)%var(ixSEG%D03_MinStorage)%dat(1)  = D03_MinStorage(ix)
     ...
     ups_trib: do ix = 1, nTribOutlet
       ...
       if (is_lake_sim) then
         ...
         structSEG_main  (nRch_mainstem+ix)%var(ixSEG%D03_MinStorage)%dat(1)  = structSEG(ixx)%var(ixSEG%D03_MinStorage)%dat(1)
 ...
 subroutine mpi_route(pid,           & ! input: proc id
  ...
  USE globalData, ONLY: RCHFLX_trib         ! tributary reach flux structure
  USE globalData, ONLY: RCHFLX_main         ! Reach flux data structures (master proc, mainstem)
  USE globalData, ONLY: RCHFLX_SUB          ! Reach flux data structures for sub-surface runoff (master proc, mainstem)
  ...
  USE globalData, ONLY: basinSubRunoff_main    ! mainstem only HRU runoff
  USE globalData, ONLY: basinSubRunoff_trib    ! tributary only HRU runoff  
  ...
  if (doesScatterRunoff) then
    ...
    if (nNodes==1) then
      ...
      if (.not.allocated(basinSubRunoff_main)) then
        ierr=10; message=trim(message)//'master proc: mainstem subsurface runoff array is not allocated'; return
      end if
      ...
      if (masterproc) then
        ...
        if (doesSubSurfRoute==1) then
          if (nHRU_mainstem > 0 .and. .not.allocated(basinSubRunoff_main)) then
            ierr=10; message=trim(message)//'mainstem subsurface runoff array is not allocated/populated'; return
          end if
          if (.not.allocated(basinSubRunoff_trib) .and. doesSubSurfRoute==1) then
            ierr=10; message=trim(message)//'master proc: tributary subsurface runoff array is not allocated'; return
          end if
        end if
        ...
        if(.not.allocated(basinSubRunoff_trib) .and. doesSubSurfRoute==1) then
          ierr=10; message=trim(message)//'tributary subsurface runoff array is not allocated/populated'; return
        end if        
        if (is_lake_sim) then
          if (.not.allocated(basinEvapo_trib)) then
  ...
  call main_route(iens,              &  ! input: ensemble index
                  ...
                  basinSubRunoff_trib,& ! input: basin subsurface runoff (m/s)
  ...
  if (masterproc) then
    ...
    call main_route(iens,                    &  ! input: ensemble index
                    ...
                    basinSubRunoff_main,     &  ! input: basin subsurface runoff (m/s)
 ...
 SUBROUTINE scatter_runoff(nNodes, comm, & ! mpi variables: number nodes, communicator
  ...
  USE globalData, ONLY: basinSubRunoff_main  ! HRU runoff holder for mainstem  
  USE globalData, ONLY: basinRunoff_trib  ! HRU runoff holder for tributary
  USE globalData, ONLY: basinSubRunoff_trib  ! HRU runoff holder for tributary  
  ...
  USE public_var, ONLY: is_lake_sim       ! logical whether or not lake should be simulated
  USE public_var, ONLY: doesSubSurfRoute  ! whether does subsurface routing  
  ...
  real(dp)                                :: basinSubRunoff_local(nHRU)   ! temporal basin runoff (m/s) for whole domain
  ...
  if (nNodes==1) then
    ...
    if (doesSubSurfRoute==1)then 
    if (.not. allocated(basinSubRunoff_main)) then
      allocate(basinSubRunoff_main(nHRU), stat=ierr)
      if(ierr/=0)then; message=trim(message)//'problem allocating array for [basinRunoff_main]'; return; endif
    end if
    basinSubRunoff_main(:) = runoff_data%basinSubRunoff(:)
    endif
    if (is_lake_sim) then
    ...
    if (masterproc) then ! this is a root process
      ...
      if (.not. allocated(basinSubRunoff_main) .and. (doesSubSurfRoute)==1) then
        allocate(basinSubRunoff_main(nHRU_mainstem), stat=ierr)
        if(ierr/=0)then; message=trim(message)//'problem allocating array for [basinSubRunoff_main]'; return; endif
      endif    
      if (is_lake_sim) then
      ...
      if (doesSubSurfRoute==1)then 
      basinSubRunoff_local(1:nHRU) = runoff_data%basinSubRunoff(1:nHRU)
      basinSubRunoff_main(1:nHRU_mainstem) = basinSubRunoff_local(1:nHRU_mainstem)
      endif
    ...
    if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    if (doesSubSurfRoute==1)then 
          call shr_mpi_scatterV(basinSubRunoff_local(nHRU_mainstem+1:nHRU), &
                          hru_per_proc(0:nNodes-1),                &
                          basinSubRunoff_trib,                        &
                          ierr, cmessage)
          if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
