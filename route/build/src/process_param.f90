...
contains
  ...
  SUBROUTINE basinUH(dt, fshape, subfshape, tscale, subtscale, IERR, MESSAGE)
  ! ---------------------------------------------------------------------------------------
  ! Creator:
  ! --------
  ...
  USE globalData,        ONLY: FRAC_FUTURE_SUB         ! fraction of subsurface runoff in future time steps
  USE public_var,        ONLY : doesSubSurfRoute        ! whether activate subsurface routing
  ...
  REAL(DP), INTENT(IN)                   :: subfshape   ! shapef parameter (subsurface runoff) in gamma distribution
  REAL(DP), INTENT(IN)                   :: subtscale   ! time scale parameter (subsurface runoff)  
  ! output
  ...
  REAL(DP)                               :: X_VALUE_SUB ! x value (subsurface runoff)
  REAL(DP)                               :: CUMPROB_SUB ! cumulative probability (subsurface runoff)
  ...
  if(CUMPROB_SUB > 0.999_dp) then ! in case if the cumprob is close to 1 in one model time step
   ...
   ntdh_try = 0.5_dp*(ntdh_min + ntdh_max)
   do itry=1,maxtry
    X_VALUE_SUB = dt*ntdh_try/subtscale
    CUMPROB_SUB = gammp(subfshape, X_VALUE_SUB)
    !print*, tscale, ntdh_try, cumprob, x_value, itry
  ...
  ntdh = ceiling(ntdh_try)
  ! allocate space for the time-delay histogram
  if (.not.allocated(FRAC_FUTURE_SUB)) then
    allocate(FRAC_FUTURE_SUB(ntdh), stat=ierr)
    if(ierr/=0)then; message=trim(message)//'unable to allocate space for the time delay histogram'; return; endif
  endif
  ! loop through time steps and compute the fraction of runoff in future time steps
  PSAVE = 0.                                                 ! cumulative probability at JTIM-1
  DO JTIM=1,NTDH
   TFUTURE                = REAL(JTIM, kind(dp))*DT       ! future time
   CUMPROB_SUB            = gammp(subfshape,TFUTURE/subtscale)   ! cumulative probability at JTIM
  ...
  END DO
  ! ensure that the fractions sum to 1.0 (account for rounding errors, and not enough bins)
  FRAC_FUTURE_SUB(:) = FRAC_FUTURE_SUB(:) / SUM(FRAC_FUTURE_SUB(:))
endif
