...
CONTAINS
 ...
 SUBROUTINE init_ntopo_data(pid,           & ! input: proc id
  ...
  USE globalData,  ONLY: RCHFLX_SUB
 ...
 SUBROUTINE init_state_data(pid, nNodes, comm, ierr, message)
  ...
  USE globalData, ONLY: RCHSTA            ! restart states
  USE globalData, ONLY: RCHFLX            ! restart states  
  ...
  USE globalData, ONLY: RCHFLX_SUB        ! subsurface routing flux  
  USE globalData, ONLY: ixRch_order      ! global reach index in the order of proc assignment (size = total number of reaches in the entire network)
  ...
  integer(i4b)                     :: iseg             ! loop index for seg
  integer(i4b)                     :: jseg             ! loop index for seg
  integer(i4b),        allocatable :: ntdh(:)          ! number of waves at each reach
  integer(i4b)                     :: nseg             ! number of tributary reaches
  character(len=strLen)            :: cmessage         ! error message of downwind routine
  ...
  if (trim(fname_state_in)/=charMissing) then
   ...
   if (nNodes>1) then
    ...
    nseg=size(RCHFLX(iens,:))
    allocate(ntdh(nSeg), stat=ierr)
    do iseg=1,nseg
      ntdh(iseg)= size(RCHFLX(iens, iseg)%QFUTURE_IRF)
    enddo
    ...
    !  jseg= ixRch_order(iseg)
     jseg= iseg
    !  print*, RCHFLX(iens,jseg)%QFUTURE(1)
    !  RCHFLX_main(iens, iseg)%QFUTURE(1:ntdh(iseg)) = RCHFLX(iens, jseg)%QFUTURE(1:ntdh(iseg))
    !  RCHFLX_main(iens, iseg)%QFUTURE_IRF(1:ntdh(iseg)) = RCHFLX(iens, jseg)%QFUTURE_IRF(1:ntdh(iseg))
     RCHFLX_main(iens, iseg)%REACH_VOL = RCHFLX(iens, jseg)%REACH_VOL
     RCHFLX_main(iens, iseg)%REACH_Q      = 0._dp
     RCHFLX_main(iens, iseg)%BASIN_QR(0)  = 0._dp
     RCHFLX_main(iens, iseg)%BASIN_QR(1)  = 0._dp
     RCHFLX_main(iens, iseg)%BASIN_QI  = 0._dp
    !  RCHSTA_main(iens, 1:nRch_mainstem)%IRF_ROUTE = RCHSTA(iens, 1:nRch_mainstem)
     enddo
    end if    
   if (nRch_trib>0) then
    do iseg=1,nRch_trib
      ! jseg= ixRch_order(iseg+nRch_mainstem)
      jseg= iseg+nRch_mainstem+1
      RCHFLX_trib(iens, iseg)%REACH_VOL = RCHFLX(iens, jseg)%REACH_VOL
      ! RCHFLX_trib(iens, iseg)%QFUTURE(1:ntdh(iseg)) = RCHFLX(iens, jseg)%QFUTURE(1:ntdh(iseg))
      ! RCHFLX_trib(iens, iseg)%QFUTURE_IRF(1:ntdh(iseg)) = RCHFLX(iens, jseg)%QFUTURE_IRF(1:ntdh(iseg))
      RCHFLX_trib(iens, iseg)%REACH_Q      = 0._dp
      RCHFLX_trib(iens, iseg)%BASIN_QI  = 0._dp
      RCHFLX_trib(iens, iseg)%BASIN_QR(0)  = 0._dp
      RCHFLX_trib(iens, iseg)%BASIN_QR(1)  = 0._dp      
      ! RCHSTA_trib(iens,:) = RCHSTA(iens, nRch_mainstem+1:nRch_mainstem+nRch_trib)
    enddo
   endif    
    end associate
